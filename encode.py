...
def encode_phrase(phrase, input_wav, output_wav="encoded_phrase_ultrasonic.wav", 
                 char_dur=0.15, gap_dur=0.08, sample_rate=44100, ultrasonic_amp=0.1):
    sr = sample_rate
    ...
    for char in phrase.upper():
        if char.isspace():
            continue
        tone = generate_tone(char, duration=char_dur, sample_rate=sr)
    ...
    sf.write(output_wav, mixed.astype(np.float32), sr)
    print(f"[ENCODE] Message encoded in '{output_wav}' at {sr} Hz")
